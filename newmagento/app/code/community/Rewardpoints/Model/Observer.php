...
class Rewardpoints_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    {   
        ...
        if(get_class($block) =='Mage_Adminhtml_Block_Customer'
            && $block->getRequest()->getControllerName() == 'customer')
        {
                ...
                if ($block->getType() == 'catalog/product_price' || strpos($block->getType(), 'product_price') !== false){
                    ...
                    //        && Mage::app()->getFrontController()->getRequest()->getControllerName() == 'category' && $show_list_info){
                        ...
                        if ($_product = $block->getProduct()){
                            ...
                            $html = $transport->getHtml();
                            $transport->setHtml($html.$extraHtml.'hi');
                        }
                    }
                }
                if(
    ...
    public function processBeforeSave($observer){
        ...
        if (Mage::getConfig()->getModuleConfig('J2t_Rewardproductvalue')->is('active', 'true') && ($object instanceof Mage_Sales_Model_Order)) {
            if (($customer_id = $object->getCustomerId()) && ($store_id = $object->getStoreId()) && (Mage::helper('rewardpoints')->isModuleActive($object->getStoreId()))){
                $reward_model = Mage::getModel('rewardpoints/stats');
                $reward_object = $reward_model->loadCustomerOrderQuote($customer_id, Rewardpoints_Model_Stats::TYPE_POINTS_REQUIRED, $object->getQuoteId());
                if ($reward_object->getId() && $reward_object->getRewardpointsStatus() != $object->getStatus() 
                        && $reward_object->getRewardpointsState() != $object->getState()){
