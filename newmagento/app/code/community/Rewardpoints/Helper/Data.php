...
class Rewardpoints_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	$img = '';
        ...
        if (!$points && $product->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_GROUPED){
            ...
            foreach ($associatedProducts as $single_product){
                ...
                $cur_points = $this->getProductPoints($single_product, true, $from_list, false, null, null, $customer_group_id);
                if($points==0 || $points>$cur_points) {                    
                    $points = $cur_points;
                }
            }
        }
    ...
    protected function getItemPoints($_item, $storeId, $money_points = false, $check_discount = false, $customer_group_id = null){
        ...
        if ($_item->getParentItem() && $_item->getParentItem()->getProduct() 
                && $_item->getParentItem()->getProduct()->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE
                && ($_item->getQty() || $_item->getQtyOrdered()) ){
        ...
        if ($_item->getParentItem() && $_item->getParentItem()->getProduct() 
                && $_item->getParentItem()->getProduct()->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE 
                && $this->canBundleChildrendRule($storeId)){ 
