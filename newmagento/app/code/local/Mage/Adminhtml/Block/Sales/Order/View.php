...
class Mage_Adminhtml_Block_Sales_Order_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    ...
    public function __construct()
    {
        ...
        if ($order->getStatus() === 'fraud') {
            ...
            $this->_addButton('order_reorder', array(
                ...
                'onclick'   => "confirmSetLocation('{$message}', '{$this->clearFraud($order)}')",
                'class'     => 'go'
            ));
        }
    }
    ...
    {   
        if(Mage::app()->getRequest()->getParam('clear')==1) {
            $order->setState(Mage_Sales_Model_Order::STATE_PROCESSING)
            ->setStatus(Mage_Sales_Model_Order::STATE_PROCESSING)
            ->save();

            Mage::app()->getFrontController()->getResponse()->setRedirect($this->getUrl('*/*'));
        }

        return $this->getUrl('*/*/*', array('clear'=>true));
    }
